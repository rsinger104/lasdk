...
var screenData = {};
var observer;
var dynamicPermissionSetCallback, dynamicPermissionMutationCallback, dynamicPermissionMutationConfig;
var dynamicPermissionMutationObserver;
...
window.AssistController = {
    ...
    startSupport : function(config) {
        ...
        if (config == false) {
            console.error("Attempted to call AssistController with corrupt config");
            return;
        }
            ...
            request.onreadystatechange = function () {
                if (request.readyState == 4) {
                    if (request.status == 200) {
                        ...
                        if (typeof window["AssistCSDK"] !== 'undefined') {
                            AssistCSDK.endCall();
                        }
    ...
    setPopupWindow: function(popupWindow) {
        console.log("got popup window");
        window.AssistCSDK = popupWindow.AssistCSDK;
    },
    ...
    start : function(config) {
        ...
        AssistController.startDynamicPermissionHandling(); 
    ...
    startDynamicPermissionHandling : function() {
        if (dynamicPermissionSetCallback && typeof dynamicPermissionSetCallback !== 'undefined') {
            dynamicPermissionSetCallback(AssistController.sourceWindow.document.body);
        };
        if (dynamicPermissionMutationCallback && typeof dynamicPermissionMutationCallback !== 'undefined') {
            dynamicPermissionMutationObserver = new MutationObserver(dynamicPermissionMutationCallback);
            dynamicPermissionMutationObserver.observe(AssistController.sourceWindow.document.body, dynamicPermissionMutationConfig);
        }
    }, 

    setupInputManager: function() {
        
        console.log("setup assist input manager");
                ...
                AssistController.rootTopic.openPrivateSubtopic({type: "input"}, function(subtopic) {
                    ...
                    subtopic.messageReceived = function(source, message) {
                        ...
                        switch (type) {
                            ...
                            console.log("Scheduling a screen update (Form filled)");
    ...
    setupDynamicPermissionHandling: function(setCallback, mutationCallback, observerConfig) {
        dynamicPermissionSetCallback = setCallback;
        dynamicPermissionMutationCallback = mutationCallback;
        dynamicPermissionMutationConfig = observerConfig;
    },
 
    setupIFrameCaptureHandler : function(config) {

        AssistIFrameCaptureHandler.init(AssistController.sourceWindow, window, 
            config.getAllowedIframeOrigins(), handler, getCommonAgentViewablePermissions, hasPermissionToView);
        ...
        function handler(changedIframeElement) {
            ...
            if (changedIframeImage) {
                console.log("Performing rerender with new iframe image");
                changedIframeImage.onload = function() {
                    var canvas = renderer( previousReplicationRenderQueue );
    ...
    endSupport : function() {
        console.log("Unloading assist window");
        ...
        if (dynamicPermissionMutationObserver && typeof dynamicPermissionMutationObserver !== 'undefined') {
            dynamicPermissionMutationObserver.disconnect();
        }
        ...
        try {
            ...
            console.error("Couldn't remove assist config");
        }
        ...
        try {
            ...
            console.error("Couldn't close shared sub-windows.");
        }
        ...
        try {
            ...
            console.error("Couldn't close root topic");
        }
        ...
        try {
            ...
            console.log("No glass pane to remove");
        }
        ...
        if (imagePreloader) {
            imagePreloader({
                complete: function(replicationImages) {
                    ...
                    var canvas = renderer( previousReplicationRenderQueue , config);

                    config.onrendered(canvas);
        ...
        try {
            if (AssistController.screenShareTopic) {
                ...
                if (partialParseElements && partialParseElements.length > 0) {
                    ...
                    console.log("Partial capture screen and send (" + partialParseElements.length + " element(s))");
                    AssistController.partialParseAndRender(config, partialParseElements);
                }
                else {
                    console.log("Capture screen and send");
                    var listenedInputElements = AssistInputManager.getListenedInputElements();
        ...
        AssistController.capture({
            ...
            onrendered : function(canvas) {
                ...
                if (AssistController.config.trace()) {
                    ...
                    console.log("Capturing took: " + ms + "ms");
                }
            ...
            ignoredElementCallback: function(element) {
                ...
                if (element.nodeName == "IFRAME") {
                    console.log("posting capture to iframe"); 
    ...
    scheduleObserver : function() {
        ...
        if (observer == null || typeof observer === 'undefined') {
            ...
            observer = new MutationObserver(function(mutations) {
                ...
                if (rerender == true) {
                    AssistController.ScreenshotQueue.resetScreenshotQueueTimeout();
                    console.log("do rerender (mutations)");
    ...
    registerScrollListeners : function() {
        ...
        function doRender(event) {
            console.log("do rerender (listeners)");
            AssistController.ScreenshotQueue.resetScreenshotQueueTimeout();
    ...
    TransitionListener: function() {
        ...
        function transitionEventHandler(event) {
            AssistController.ScreenshotQueue.resetScreenshotQueueTimeout();
            console.log("do rerender (transition completed)");
    ...
    createGlassPane : function() {
        ...
        function resizeUpdate() {
            ...
            AssistController.ScreenshotQueue.resetScreenshotQueueTimeout();
            console.log("do rerender (resize)");
            AssistController.ScreenshotQueue.queueScreenshot();
        }
    ...
    connectWithVoiceVideo : function(configuration) {
        ...
        if (AssistCSDK.initRun == true) { // subsequent page/reconnect with csdk path
            console.log("reconnect csdk");
            AssistCSDK.reconnect(AssistUtils, console, configuration, AssistController.sourceWindow, 
                AssistControllerInterface, onAgentVideoWindowCreated);

            AssistController.connectWebSocket(configuration);
            
        } else { // initial page/init with csdk path
            AssistCSDK.init(AssistUtils, console, configuration, AssistController.sourceWindow, 
                AssistControllerInterface, onAgentVideoWindowCreated,

                function onInCall() { // callback
                    AssistController.connectWebSocket(configuration);
                }
            );
        }
    ...
    connectWebSocket : function(configuration) {
        ...
        console.log(configuration.getUrl());
        console.log(configuration.getCorrelationId());
        ...
        AssistAED.connectRootTopic(configuration.getCorrelationId(), function(rootTopic) {
            ...
            rootTopic.participantJoined = function(newParticipant) {
                if (newParticipant.metadata.role == "agent") {
                    ...
                    if (typeof window["AssistCSDK"] !== 'undefined') {
                        AssistCSDK.setAgentName(newParticipant.metadata.name);
                        AssistCSDK.setAgentPicture(newParticipant.metadata.avatar);
                    }
            ...
            rootTopic.connectionLost = function() {
            	console.log("Cobrowse connection permanently lost.");
                if (pageHasCobrowsingClass() == true) {
                	AssistController.doSDKCallback("onCobrowseInactive", defaultOnCobrowseInactive);
                	removeCobrowsingClass();
                }
   ...
   startScreenShare : function(configuration, initialPermissions) {
       // TODO we might be OK to remove this now
        ...
        AssistController.rootTopic.openPrivateSubtopic({"type":"shared-window", "interactive":"true", "scrollable":"true"}, function(newTopic) {
            ...
            newTopic.participantJoined = function(newScreenParticipant) {
                ...
                if (newScreenParticipant.metadata.role == "agent") {
                    ...
                    AssistController.ScreenshotQueue.resetScreenshotQueueTimeout();
                    console.log("do rerender (new participant), count= " + AssistController.screenShareTopic.participants.length);
                    AssistController.screenShareWindow.sendSizeAndPosition();
                    AssistController.ScreenshotQueue.queueScreenshot(undefined, true); 
                }
            ...
            newTopic.participantLeft = function(removedParticipant) {
                ...
                console.log("Participant left, remaining= " + AssistController.screenShareTopic.participants.length);
            ...
            if (AssistController.supportDiv) {
                ...
                console.log("Creating draggable-agent-window (2) with scaleFactor: " + scaleFactor);
        ...
        AssistController.rootTopic.subtopicClosed = function(closedTopic) {
        	if (closedTopic.metadata.type == "shared-window") {
        		console.log("shared-window topic closed, reopening.");
    ...
    allowCobrowseForAgent : function(agent) {
        ...
        if (AssistController.screenShareTopic && AssistController.screenshotInProgress == false) {
            console.log("Allowing co-browse for agent " + agentIdentifier);
            AssistController.screenShareTopic.updatePermission(AssistAED.PERMISSIONS.ALLOWED, agent);
        } else {
            console.log("Co-browse not initialised, deferring allowing co-browse permission for agent " + agentIdentifier);
        ...
        if (AssistController.inputTopic) {
            console.log("Allowing form filling access for agent " + agentIdentifier);
    ...
    disallowCobrowseForAgent : function(agent) {
        var agentIdentifier = agent.name + "(" + agent.id + ")";
        if (AssistController.screenShareTopic) {
            console.log("Rejecting co-browse for agent " + agentIdentifier);
            AssistController.screenShareTopic.updatePermission(AssistAED.PERMISSIONS.DENIED, agent);
        } else {
            console.log("Co-browse not initialised, deferring rejecting co-browse permission for agent " + agentIdentifier);
    ...
    setScreenshotInProgress: function(inProgress) {
        ...
        if (pendingAgentPermissionChanges > 0) {
            console.log("Setting previously requested co-browse permission for " + pendingAgentPermissionChanges + " agents.");
    ...
    resumeCobrowse : function() {
        console.log("Co-browse un-paused.");
        if (AssistController.cobrowsePaused && AssistController.screenShareTopic){
            AssistController.sendOnAssistPagesMessage();
            AssistController.ScreenshotQueue.resetScreenshotQueueTimeout();
            AssistController.ScreenshotQueue.queueScreenshot();
